...
def _parse_arguments():
    ...
    parser.add_argument("--task", required=True, type=str, help="The task that is being evaluated.", choices=["jblimp", "blimp", "ewok", "entity_tracking", "wug_adj", "wug_past", "comps", "vqa", "winoground"])
...
def main():
    ...
    if "epoch" in args.model_path_or_name:
        args.model_name = pathlib.Path(args.model_path_or_name).parent.name
        checkpoint_name = pathlib.Path(args.model_path_or_name).name
    else:
